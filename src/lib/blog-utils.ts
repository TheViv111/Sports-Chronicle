...
export const transformBlogPostForDisplay = (
  ...
  const translations = (post as any).translations || {};
  const lang = currentLanguage || post.language;

  const translatedTitle = translations?.title?.[lang] ?? post.title;
  const translatedContent = translations?.content?.[lang] ?? post.content;
  const translatedExcerpt = translations?.excerpt?.[lang] ?? post.excerpt;
  const translatedCategory = translations?.category?.[lang] ?? post.category;
  ...
  if (lang === 'hi' && isFalseNinePost) {
    ...
    titleForDisplay = translations?.title?.[lang] ?? 'फॉल्स नाइन: आधुनिक फुटबॉल में भूमिका और रणनीति';
    excerptForDisplay =
      translations?.excerpt?.[lang] ??
      'फॉल्स नाइन की अवधारणा, पोज़िशनिंग, मूवमेंट और टीम संरचना पर प्रभाव — एक व्यावहारिक विश्लेषण।';
    displayCategoryForBadge = translations?.category?.[lang] ?? 'फुटबॉल';
    contentForDisplay =
